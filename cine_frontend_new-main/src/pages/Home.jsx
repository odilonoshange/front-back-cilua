    ...
    <div className="flex flex-col min-h-screen">
      <section className="relative h-[80vh] w-full">
        ...
        <div className="relative z-10 container mx-auto flex h-full flex-col justify-end px-4 pb-24 md:pb-32">
          ...
          <p className="mb-8 max-w-2xl text-lg text-white/80 md:text-xl">
            Descubra produções de Luanda, conheça filmes e espetáculos de teatro e acompanhe a cultura audiovisual nacional.
          </p>
      ...
      <section className="bg-surface/30 py-16">
        <div className="container mx-auto px-4">
          <h2 className="mb-8 text-2xl font-bold md:text-3xl">Exibição Online</h2>
      ...
      <section className="container mx-auto px-4 py-16">
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-6">
          {['Drama', 'Comédia', 'Ação', 'Documentário', 'Teatro Clássico', 'Stand-up'].map((cat, i) => (
            <Link key={i} to={`/explorar?categoria=${encodeURIComponent(cat)}`} className="flex h-24 items-center justify-center rounded-lg border border-white/10 bg-surface/50 px-4 text-center font-medium transition-colors hover:bg-white/10 hover:text-white text-muted">
              {cat}
            </Link>
          ))}
        </div>
